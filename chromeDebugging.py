...
def run_reboot(dut_ip, username="root", password="test0000", reboot_wait_time=60):
    ...
    if check_if_remote_system_is_live(dut_ip):
        sshpassCmd = "sshpass -p " + password + " ssh " + username + "@" + dut_ip + " 'reboot'"
        print (sshpassCmd)
        p = subprocess.Popen(sshpassCmd, stdout=subprocess.PIPE, shell=True)
        time.sleep(3)
        if check_if_remote_system_is_live(dut_ip):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--search_for', dest='search_patterns', help='provide one or many searchstrings with spaces in between', nargs='+')
